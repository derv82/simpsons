...
def fix_wiki_text(text):
	...
	text = text.replace('\\"', '"')
	while '<'  and '>'  in text: text = text[:text.find('<')] + text[text.find('>')+1:]
...
def get_wikipedia(episode):
	...
	if '==Plot==' in r: plot = fix_wiki_text(web.between(r, '==Plot==', '==')[0])
...
def get_season(season):
	...
	chunks = web.between(r, '<table border="0" cellpadding="0" cellspacing="0"><tr>', '</table>')
	episode = 1
	for chunk in chunks:
		...
		code  = fields[12].strip().replace('(', '').replace(')', '').replace('#', '')
		if ' /' in code: code = code[:code.find(' /')]
		sdate = fields[18].strip()
...
if __name__ == '__main__':
	...
	for season in xrange(1, len(episode_counts)+1):
